...
class Job(object):
    ...
    def find_jobs(self, **kwargs):
        ...
        try:
            ...
            raise genie2.exception.genie_exception.GenieException(
                'Unable to get jobs due to error code: [ '
                + str(http_error.code)
                + '] because ['
                + http_error.read()
                + ']', http_error.code)
        except urllib2.URLError, url_error:
            raise genie2.exception.genie_exception.GenieException('Unable to get job. ' + url_error.reason)
    ...
    def get_job(self, job_id):
        ...
        try:
            ...
            raise genie2.exception.genie_exception.GenieException(
                'Unable to get job due to error code: [ '
                + str(http_error.code)
                + '] because ['
                + http_error.read()
                + ']', http_error.code)
            ...
            raise genie2.exception.genie_exception.GenieException(
                'Unable to get job. ' + url_error.reason)
    ...
    def submit_job(self, job_obj):
        ...
        try:
            ...
            raise genie2.exception.genie_exception.GenieException(
                'Unable to submit job due to error code: [ '
                + str(http_error.code)
                + '] because ['
                + http_error.read()
                + ']', http_error.code)
