...
class Cluster(object):
    ...
    def create_cluster(self, cstr_obj):
        ...
        try:
            ...
            raise GenieException('Unable to create cluster due to error code: ['
                        + str(http_error.code)
                        + '] because ['
                        + http_error.read()
                        + ']',http_error.code)
        except urllib2.URLError, url_error:
            raise GenieException('Unable to create cluster. ' + url_error.reason)
    ...
    def update_cluster(self,cstr_id, cstr_obj):
        ...
        try:
            ...
            raise GenieException('Unable to update cluster due to error code: ['
                        + str(http_error.code)
                        + '] because ['
                        + http_error.read()
                        + ']',http_error.code)
        except urllib2.URLError, url_error:
            raise GenieException('Unable to update cluster. ' + url_error.reason)
    ...
    def delete_cluster(self, cstr_id):
        ...
        try:
            ...
            raise GenieException('Unable to delete cluster due to error code: ['
                        + str(http_error.code)
                        + '] because ['
                        + http_error.read()
                        + ']',http_error.code)
        except urllib2.URLError, url_error:
            raise GenieException('Unable to delete cluster. ' + url_error.reason)
    ...
    def get_cluster(self, cstr_id):
        ...
        try:
            ...
            raise GenieException('Unable to get cluster due to error code: ['
                        + str(http_error.code)
                        + '] because ['
                        + http_error.read()
                        + ']',http_error.code)
        except urllib2.URLError, url_error:
            raise GenieException('Unable to get cluster. ' + url_error.reason)
    ...
    def get_clusters(self,**params):
        ...
        try:
            ...
            raise GenieException('Unable to get all clusters due to error code: ['
                        + str(http_error.code)
                        + '] because ['
                        + http_error.read()
                        + ']',http_error.code)
        except urllib2.URLError, url_error:
            raise GenieException('Unable to get all clusters. ' + url_error.reason)
    ...
    def cluster_is_configured(self, cluster_id):
        ...
        try:
            ...
            if(http_error.code == 404):
                ...
                raise GenieException(http_error.read())
        except urllib2.URLError, url_error:
            raise GenieException(url_error.reason)
    ...
    def add_commands_to_cluster(self, cluster_id, command_ids):
        ...
        try:
            if command_ids is None \
                    ...
                    or not command_ids:
                raise GenieException("Command ids is not valid. Unable to continue.")
            if not cluster_id:
                raise GenieException("No cluster id entered. Unable to continue.")
            ...
            raise GenieException(http_error.read(),http_error.code)
        except urllib2.URLError, url_error:
            raise GenieException(url_error.reason)
    ...
    def add_tags_to_cluster(self, cluster_id, tags):
        ...
        try:
            if tags is None \
                    ...
                    or not tags:
                raise GenieException("Tag is not a valid list. Unable to continue.")
            if not cluster_id:
                raise GenieException("No cluster id entered. Unable to continue.")
    ...
    def remove_tag_from_cluster(self, cluster_id, tag):
        ...
        try:
            if tag is None:
                raise GenieException("Tag cannot be None. Unable to continue.")
            if not cluster_id:
                raise GenieException("No cluster id entered. Unable to continue.")
