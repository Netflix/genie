...
/**
 * Converts {@link JobDirectoryManifest} from/to {@link JobFileManifestMessage} in order to transport manifests
 * over gRPC.
 *
 * @author mprimi
 * @since 4.0.0
...
public class JobDirectoryManifestProtoConverter {
    ...
    /**
     * Construct a {@link JobFileManifestMessage} from the given {@link JobDirectoryManifest}.
     *
     * @param claimedJobId the id of the job this file manifest belongs to
     * @param manifest     the manifest
     * @return a {@link JobFileManifestMessage}
     * @throws GenieConversionException if conversion fails
     */
    public JobFileManifestMessage manifestToProtoMessage(
        ...
        return JobFileManifestMessage.newBuilder()
            .setJobId(claimedJobId)
            .setManifestJsonString(manifestJsonString)
            .build();
    }

    /**
     * Load a {@link JobDirectoryManifest} from a {@link JobFileManifestMessage}.
     *
     * @param message the message
     * @return a {@link JobDirectoryManifest}
     * @throws GenieConversionException if loading fails
     */
    public JobDirectoryManifest toManifest(final JobFileManifestMessage message) throws GenieConversionException {
        try {
            return objectMapper.readValue(message.getManifestJsonString(), JobDirectoryManifest.class);
