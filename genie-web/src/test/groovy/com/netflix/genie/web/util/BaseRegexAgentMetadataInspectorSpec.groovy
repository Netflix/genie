...
class BaseRegexAgentMetadataInspectorSpec extends Specification {
    ...
    def "Decide #decisionIfMatch.name() if there is a match, handle corner cases and errors"() {
        ...
        AgentFilterService.InspectionReport decision

        when:
        decision = inspector.inspectWithPattern("^foo\$", "")

        then:
        decision.getDecision() == InspectionDecision.CONTINUE
        ...
        where:
        decisionIfMatch           | _
        InspectionDecision.ACCEPT | _
        InspectionDecision.REJECT | _
    }

    class TestRegexInspector extends BaseRegexAgentMetadataInspector {

        TestRegexInspector(
            final InspectionDecision decisionIfMatch
        ) {
            super(decisionIfMatch)
        }

        @Override
        AgentFilterService.InspectionReport inspect(final AgentClientMetadata agentClientMetadata) {
