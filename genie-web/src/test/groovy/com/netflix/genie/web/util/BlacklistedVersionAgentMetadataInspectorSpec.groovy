...
class BlacklistedVersionAgentMetadataInspectorSpec extends Specification {
    ...
    def "Match #agentVersion against #blacklistExpression and expect #expectedDecision"() {
        ...
        "1.2.3"      | "^(1\\.2\\.3|1\\.2\\.4)\$"  | InspectionDecision.REJECT
        "1.2.3-RC.2" | "^(1\\.2\\.3|1\\.2\\.4)\$"  | InspectionDecision.CONTINUE
        "1.2.0"      | "^(1\\.2\\.3|1\\.2\\.4)\$"  | InspectionDecision.CONTINUE
        "1.2.0"      | "^(1\\.2\\.3|1\\.2\\.4)\$"  | InspectionDecision.CONTINUE
        "1.2.0"      | "1\\.2\\..*"                | InspectionDecision.REJECT
        "1.2.0-RC.2" | "1\\.2\\..*"                | InspectionDecision.REJECT
    }
}
