...
class BlacklistedVersionAgentMetadataInspectorSpec extends Specification {
    ...
    def "Match #agentVersion against #blacklistExpression and expect #expectedDecision"() {
        ...
        "1.2.3"      | "^(1\\.2\\.3|1\\.2\\.4)\$" | InspectionReport.Decision.REJECT
        "1.2.3-RC.2" | "^(1\\.2\\.3|1\\.2\\.4)\$" | InspectionReport.Decision.ACCEPT
        "1.2.0"      | "^(1\\.2\\.3|1\\.2\\.4)\$" | InspectionReport.Decision.ACCEPT
        "1.2.0"      | "^(1\\.2\\.3|1\\.2\\.4)\$" | InspectionReport.Decision.ACCEPT
        "1.2.0"      | "1\\.2\\..*"               | InspectionReport.Decision.REJECT
        "1.2.0-RC.2" | "1\\.2\\..*"               | InspectionReport.Decision.REJECT
    }
}
