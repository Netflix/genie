...
function copyFiles {
    ...
    i=0
    retVal=0
    echo "$(date +%F_%T) Copying files $SOURCE to $DESTINATION"
    while true
    do
        ...
        if [ "$retVal" -eq 0 ]; then
            ...
            echo "$(date +%F_%T) Will retry in 5 seconds to ensure that this is not a transient error"
            sleep 5
            i=$(($i+1))
        fi
        ...
        if [ "$i" -eq "$NUM_RETRIES" ]; then
            echo "$(date +%F_%T) Failed to copy files from $SOURCE to $DESTINATION"
            break
        fi  
    done
...
function executeCommand {
    ...
    if [ -n "$HADOOP_HOME" ]; then
        echo "$(date +%F_%T) Copying local Hadoop config files..."
        cp $HADOOP_HOME/conf/* $CURRENT_JOB_CONF_DIR
        checkError 203
        echo "$(date +%F_%T) Copied local hadoop files from conf directory"
	echo $'\n'
    fi
    ...
    echo "$(date +%F_%T) Setting env variables by sourcing env files for resources"
    setEnvVariables
    checkError 205
    echo "$(date +%F_%T) Done setting env variables"
    echo $'\n'

    # Assuming cluster config files HAVE to be specified
    echo "$(date +%F_%T) Copying cluster Config files ..."
    copyFiles "$S3_CLUSTER_CONF_FILES" "file://$CURRENT_JOB_CONF_DIR"/
    checkError 206
    echo "$(date +%F_%T) Copied cluster config files"
    echo $'\n'
    
    if [ -n "$S3_COMMAND_CONF_FILES" ]; then
        echo "$(date +%F_%T) Copying command Config files ..."
        copyFiles "$S3_COMMAND_CONF_FILES" "file://$CURRENT_JOB_CONF_DIR"/
        checkError 207
        echo "$(date +%F_%T) Copied command config files"
        echo $'\n'
    fi
    
    if [ -n "$S3_APPLICATION_CONF_FILES" ]; then
        echo "$(date +%F_%T) Copying application Config files ..."
        copyFiles "$S3_APPLICATION_CONF_FILES" "file://$CURRENT_JOB_CONF_DIR"/
        checkError 208
        echo "$(date +%F_%T) Copied application config files"
        echo $'\n'
    fi

    if [ -n "$S3_APPLICATION_JAR_FILES" ]; then
        echo "$(date +%F_%T) Copying application jar files ..."
        copyFiles "$S3_APPLICATION_JAR_FILES" "file://$CURRENT_JOB_JAR_DIR"/
        checkError 209
        echo "$(date +%F_%T) Copied application jars files"
    fi

    echo "$(date +%F_%T) Copying job dependency files: $JOB_FILE_DEPENDENCIES" 
    ...
    then
        ...
        checkError 210
        echo "$(date +%F_%T) Copied job dependency files"
        echo $'\n'
    fi
    ...
    if [ -n "$CORE_SITE_XML_ARGS" ]; then
        echo "$(date +%F_%T) Updating core-site xml args"
        updateCoreSiteXml
        checkError 204
        echo "$(date +%F_%T) Updated core-site xml args"
        echo $'\n'
    fi
    
    mkdir tmp
    echo "$(date +%F_%T) Executing CMD: $CMD $@"
...
function appendKeyValueToCoreSite {
    ...
    echo "$(date +%F_%T) Appending $1/$2 to core-site.xml as key/value pair."
    KEY=$1
    VALUE=$2
...
function archiveToS3 {
    ...
    then
        echo "$(date +%F_%T) Not archiving files in working directory"
        return
    fi

    S3_PREFIX=$S3_ARCHIVE_LOCATION/`basename $PWD`
    echo "$(date +%F_%T) Archiving all files in working directory to $S3_PREFIX"
    ...
    if [ "$?" -ne 0 ]; then
        echo "$(date +%F_%T) Failed to archive logs to tarball - but moving on"
        return
    fi
    ...
    if [ "$?" -ne 0 ]; then
        echo "$(date +%F_%T) Failed to archive logs to S3 - but moving on"
    fi
}
...
echo "$(date +%F_%T) Job Execution Env Variables"
echo "$(date +%F_%T) ****************************************************************"
env
echo "$(date +%F_%T) ****************************************************************"
echo $'\n'

echo "$(date +%F_%T) Creating job jar dir: $CURRENT_JOB_JAR_DIR"
mkdir -p $CURRENT_JOB_JAR_DIR 
checkError 201
echo "$(date +%F_%T) Job Jar dir created"
echo $'\n'

echo "$(date +%F_%T) Creating job conf dir: $CURRENT_JOB_CONF_DIR"
mkdir -p $CURRENT_JOB_CONF_DIR 
checkError 202
echo "$(date +%F_%T) Job conf directory created"
echo $'\n'

# Uncomment the following if you want Genie to create users if they don't exist already
#echo "$(date +%F_%T) Create user.group $USER_NAME.$GROUP_NAME, if it doesn't exist already"
#sudo groupadd $GROUP_NAME
#sudo useradd $USER_NAME -g $GROUP_NAME
#echo "$(date +%F_%T) User and Group created"
#echo $'\n'
