...
/**
 * Specifications for {@link ContainerImage}.
 *
 * @author tgianos
 */
class ContainerImageSpec extends Specification {

    @Unroll
    def "can validate for #name, #tag, #arguments"() {
        when:
        def image0 = new ContainerImage.Builder()
            .withName(name)
            .withTag(tag)
            .withArguments(arguments)
            .build()

        def image1 = new ContainerImage.Builder()
            .withName(name)
            .withTag(tag)
            .withArguments(arguments)
            .build()

        def image2 = new ContainerImage.Builder()
