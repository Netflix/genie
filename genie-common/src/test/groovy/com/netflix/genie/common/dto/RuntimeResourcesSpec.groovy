...
/**
 * Specifications for {@link RuntimeResources}.
 *
 * @author tgianos
 */
class RuntimeResourcesSpec extends Specification {

    @Unroll
    def "can validate for #cpu, #gpu, #memoryMb, #diskMb, #networkMbps"() {
        when:
        def computeResources0 = new RuntimeResources.Builder()
            .withCpu(cpu)
            .withGpu(gpu)
        ...
        def computeResources1 = new RuntimeResources.Builder()
            .withCpu(cpu)
            .withGpu(gpu)
            .withMemoryMb(memoryMb)
            .withDiskMb(diskMb)
