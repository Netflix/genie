...
This separation allows treating every job the same way, wether they are Spark or Notebook kernels, or anything else.
...
Out of the box, Genie supports MySQL and Postgress persistence via JPA and Hibernate.
...
This hooks are provided in order to plug-in custom logic:
